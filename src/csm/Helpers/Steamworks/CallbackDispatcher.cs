...
namespace CSM.Helpers.Steamworks {
	public static class CallbackDispatcher {
		...
		public static void ExceptionHandler(Exception e) {
			Log.Error("Steamworks CallbackDispatcher exception: ", e);
        }
    }
	...
	public sealed class Callback<T> : IDisposable {
		...
		public void Register(DispatchDelegate func) {
			...
			SteamHelpers.SteamAPI_RegisterCallback(m_pCCallbackBase.AddrOfPinnedObject(), CallbackIdentities.GetCallbackIdentity(typeof(T)));
		}

		public void Unregister() {
			// k_ECallbackFlagsRegistered is removed by SteamAPI_UnregisterCallback.
			SteamHelpers.SteamAPI_UnregisterCallback(m_pCCallbackBase.AddrOfPinnedObject());
		}
	...
	public sealed class CallResult<T> : IDisposable {
		...
		public void Set(SteamAPICall_t hAPICall, APIDispatchDelegate func = null) {
			...
			if (m_hAPICall != SteamAPICall_t.Invalid) {
				SteamHelpers.SteamAPI_UnregisterCallResult(m_pCCallbackBase.AddrOfPinnedObject(), (ulong)m_hAPICall);
			}

			m_hAPICall = hAPICall;

			if (hAPICall != SteamAPICall_t.Invalid) {
				SteamHelpers.SteamAPI_RegisterCallResult(m_pCCallbackBase.AddrOfPinnedObject(), (ulong)hAPICall);
			}
		}
