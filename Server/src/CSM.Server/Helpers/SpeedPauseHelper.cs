    ...
    public static class SpeedPauseHelper
    {
        ...
        public static void SimulationStep()
        {
            ...
            if (allowNegotiation)
            {
                ...
                if ((SpeedTarget.HasValue && SpeedTarget.Value != Speed) ||
                    (PauseTarget.HasValue && PauseTarget != Paused))
                {
                    PlayPauseSpeedChanged(PauseTarget ?? Paused,
                        SpeedTarget ?? Speed);
                }
            }
        ...
        public static void SpeedPauseResponseReceived(long highestGameTime, long highestLatency)
        {
            ...
            DateTime pauseTime = new DateTime(highestGameTime);
        ...
        {
            if (_state == SpeedPauseState.WaitingForPause)
            {
                SetPauseStateInternal(true);
                _state = SpeedPauseState.PausedWaiting;
                SendReached();
            }
            else if (_state == SpeedPauseState.WaitingForSpeedChange)
            {
        ...
        {
            // Add 1 for compatibilitiy.
            // Client expects a minimum of 2 clients.
            int numClients = 1 + MultiplayerManager.Instance.CurrentServer.ConnectedPlayers
                        .Count(p => p.Value.Status == ClientStatus.Connected);

            _ = Task.Factory.StartNew(() =>
            {
                Log.Info("Waiting for current time.");
        ...
        private static TaskCompletionSource<long> _timeTask = new TaskCompletionSource<long>();

        internal static void SetCurrentTime(long currentTime)
        {
            _currentTime = currentTime;
            _timeTask.TrySetResult(currentTime);
        }
